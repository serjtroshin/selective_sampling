...
def filter_out_invalid_responses(responses: List[str]):
    new_responses = []
    for response in responses:
        if response != INCORRECT_RESPONSE_LABEL:
            new_responses.append(response)
    if len(new_responses) == 0:
        new_responses.append(INCORRECT_RESPONSE_LABEL)
    return new_responses


# invalid responses are marked with this label
