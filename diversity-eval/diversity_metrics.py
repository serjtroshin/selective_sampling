...
class DistinctNgrams(metric.DiversityMetric):
    ...
    def __call__(self, response_set):
        ...
        response_set = utils.filter_out_invalid_responses(response_set)
...
class SentBert(metric.Similarity2DiversityFromFileMetric):
    ...
    def __init__(self, config):
        ...
        self.device = "cuda:0"   # to ensure we load on gpu
    ...
    def calc_scores(self):
        ...
        model = sentence_transformers.SentenceTransformer(model_name).to(self.device)
        resp_list = []
        metric_list = []
        # read resps
        with open(self.config['input_path'], 'r+', encoding='utf-8') as f_in:
            ...
            metric_keys = sorted([s for s in reader.fieldnames if s.startswith('metric_')])
            for row in reader:
                resps = [v for k, v in row.items() if k in resp_keys]
                metrics = [float(v) for k, v in row.items() if k in metric_keys]
                resp_list += resps
                metric_list.append(metrics)
        # calc embeds
        embeds = np.array(model.encode(resp_list, device=self.device, show_progress_bar=True)) # [ num_contexts * samples_per_context, embed_dim]
        ...
        for set_i in range(self.config['num_sets']):
            for sample_i in range(self.config['samples_per_set']):
                for sample_j in range(sample_i):
                    # adding relative diversity weights
                    similarity_scores_list.append(self.similarity_metric(embeds[set_i, sample_i, :], embeds[set_i, sample_j, :]) * metric_list[set_i][sample_i] * metric_list[set_i][sample_j])
