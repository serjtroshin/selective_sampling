...
class TopPLogitsProcessor:
    def __init__(self, top_p: float):
        ...
        self.top_p = torch.tensor(top_p).unsqueeze(0)
        self.top_k = torch.tensor(0).unsqueeze(0)

    def __call__(self, input_ids, logits):
        if logits.dim() == 1:
            logits = logits.unsqueeze(0)
        self.top_k = self.top_k.to(logits.device)
        self.top_p = self.top_p.to(logits.device)
        return self.sampler.forward_native(logits, {}, True, self.top_k, False, self.top_p).squeeze(0)
...
if __name__ == "__main__":
    ...
    print(top_p_processor(None, logits))
